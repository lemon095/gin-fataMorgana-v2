...
const (
	GroupBuyStatusPending   = "pending"   // 待开始
	GroupBuyStatusActive    = "active"    // 进行中
	GroupBuyStatusSuccess   = "success"   // 拼单成功
	GroupBuyStatusFailed    = "failed"    // 拼单失败
	GroupBuyStatusCancelled = "cancelled" // 已取消
	GroupBuyStatusExpired   = "expired"   // 已过期
)
...
type GroupBuy struct {
	...
	Status                string    `json:"status" gorm:"not null;size:20;default:'pending';index;comment:拼单状态"`
	ParticipantUid        string    `json:"participant_uid" gorm:"size:8;index;comment:参与用户ID"`
	Description           string    `json:"description" gorm:"type:text;comment:拼单描述"`
	Rules                 string    `json:"rules" gorm:"type:text;comment:拼单规则"`
	AutoStart             bool      `json:"auto_start" gorm:"not null;default:false;comment:是否自动开始"`
	Complete              string    `json:"complete" gorm:"not null;size:20;default:'pending';comment:完成状态: pending/success/cancelled"`
