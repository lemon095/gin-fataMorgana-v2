...
CREATE TABLE IF NOT EXISTS `group_buys` (
  ...
  `uid` varchar(8) NOT NULL COMMENT '用户唯一ID',
  `group_buy_type` varchar(20) NOT NULL COMMENT '拼单类型: like-点赞, share-转发, follow-关注, favorite-收藏',
  `per_person_amount` decimal(15,2) NOT NULL COMMENT '每人金额',
  ...
  `status` varchar(20) NOT NULL DEFAULT 'pending' COMMENT '拼单状态: pending-进行中, completed-已完成, expired-已过期',
  `deadline` timestamp NOT NULL COMMENT '截止时间',
  `creator_uid` varchar(8) NOT NULL COMMENT '创建者ID',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
